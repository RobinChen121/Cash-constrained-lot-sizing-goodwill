...
Aineq = [diag(-M1), diag(ones(1, T)), zeros(T, T), zeros(T, T),zeros(T, T); % yt<=Mxt
    ...
    zeros(T, 2 * T), cycleBeta, diag(ones(1, T)), M2 * tempOne;    %Edt¡Üdt-beita*w(t-1)+M2*(1-¦Ät)
    zeros(T, 2 * T), -cycleBeta, -diag(ones(1, T)), M2 * tempOne;    %Edt>=dt-beita*w(t-1)+M2*(1-¦Ät)-sigma
...
[x, fval, ~, ~] = cplexmilp (f, Aineq, bineq, Aeq, beq,...
    [ ], [ ], [ ], lb, ub, ctype, [ ], options);
toc;
fprintf ('Solution value = %f \T', - fval + B0 - BL*(1+rL)^TL);
toc
