...
if thisL > 0
    b(thisL : n - 1) = b(thisL : n - 1) - ratePay;
end
...
%% effective demand constraint, from n + xSum th to n + xSum + n - 1 th
Ed=d;
if setupIndex(1)>1
    [Ed(1:setupIndex(1)-1),~,wFlow]=ComputeEd(zeros(1,setupIndex(1)-1),d(1:setupIndex(1)-1),iniW,beita);
    iniW=wFlow(end);
end
for i=1:setupIndex(1)-1
    A(n+xSum+i-1)=1; %需求满足量为0
end
Ed(setupIndex(1))=max(0,d(setupIndex(1))-beita*iniW);
preIndexLossMuch=setupIndex(1)-1;i=setupIndex(1);
while i<=n
    if i+1<=n 
        rowIndex=n+xSum+i-1;
        if isLossMuch(i+1)~=1
            tempZhengfu=BeitaZhengfu(i-preIndexLossMuch,beita);
            A(rowIndex,preIndexLossMuch+1:i)=tempZhengfu; b(rowIndex)=Ed(preIndexLossMuch+1:i)*tempZhengfu';
        else
            tempZhengfu=BeitaZhengfu(i-preIndexLossMuch,beita);
            A(rowIndex,preIndexLossMuch+1:i)=tempZhengfu; b(rowIndex)=Ed(preIndexLossMuch+1:i)*tempZhengfu'-2*d(i+1); % i 跟 i+1 的有效需求约束
            A(rowIndex+1,i+1)=1;preIndexLossMuch=i+1;i=i+1;
        end
    end
    i=i+1;
end
if isLossMuch(n)~=1
    tempZhengfu=BeitaZhengfu(n-preIndexLossMuch,beita);
    A(n+xSum+n-1,preIndexLossMuch+1:n)=tempZhengfu; b(n+xSum+n-1)=Ed(preIndexLossMuch+1:n)*tempZhengfu';
end

%% can not lose more than the previous plan
